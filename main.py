...
from app.models import (
    user, password_reset_token, refresh_token, alarm, alarm_trigger, challenge,
...
# --- Background dispatcher: checks every active alarm on a timer ---
...
async def _dispatcher_loop():
    while True:
        try:
            fired = await asyncio.to_thread(check_and_fire_alarms)
            if fired:
                logger.info(f"Dispatcher fired {len(fired)} trigger(s)")
        except Exception as e:
            logger.error(f"Dispatcher tick failed: {e}")
