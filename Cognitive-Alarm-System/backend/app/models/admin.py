from sqlalchemy import Boolean, Column, Enum, ForeignKey, String, Text, TIMESTAMP, text
...
class CoachAssignment(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    assigned_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=text("CURRENT_TIMESTAMP"))
