...
export const HabitScoreDashboardScreen: React.FC = () => {
  ...
  const getScoreTier = (score: number) => {
    if (score >= 75) return { label: '⚡ Apex Wake-Up', color: '#34D399', bg: '#1A382B' };
    if (score >= 50) return { label: '🔥 Solid Performance', color: '#A58BFF', bg: '#2C2243' };
    if (score >= 25) return { label: '📈 Improving Latency', color: '#FBBF24', bg: '#3D2E14' };
    return { label: '🌱 Building Routine', color: '#C4B5FF', bg: '#262036' };
  };

  const formatDifficultyColor = (diff: string) => {
    switch ((diff || '').toLowerCase()) {
      case 'hard': return { bg: '#451A1A', text: '#F87171' };
      case 'medium': return { bg: '#2C2243', text: '#A58BFF' };
      default: return { bg: '#1A382B', text: '#34D399' };
    }
  };
    ...
    <ThemeBackground>
      <View style={styles.container}>
        ...
        >
          ...
          <Card style={[styles.heroCard, { backgroundColor: 'rgba(26, 22, 38, 0.95)', borderColor: 'rgba(165, 139, 255, 0.25)' }]}>
            <Card.Content style={styles.heroContent}>
              <Text style={[styles.heroSub, { color: theme.colors.onSurfaceVariant }]}>Cognitive Habit Performance</Text>
              <View style={styles.scoreRow}>
                <Text style={[styles.heroScore, { color: '#A58BFF' }]}>
