...
export const AlarmListScreen: React.FC<AlarmListScreenProps> = ({ navigation }) => {
  ...
  const startSimulation = (alarm: any) => {
    navigation.navigate('Challenge', { alarmId: alarm.alarm_id });
  };
    ...
    return (
      <Card style={[styles.card, { backgroundColor: 'rgba(26, 22, 38, 0.85)', borderColor: 'rgba(165, 139, 255, 0.15)' }]}>
        <Card.Content style={styles.cardContent}>
          ...
          >
            <Text style={[styles.repeatLabelHeader, { color: theme.colors.onSurfaceVariant }]}>
              {getRepeatLabel(item.repeat_days)}
            </Text>
            <Text style={[styles.time, { color: '#FFFFFF' }]}>{item.alarm_time}</Text>
  ...
  return (
    <ThemeBackground>
      <View style={styles.container}>
        ...
        <FAB
          icon="plus"
          style={[styles.fab, { backgroundColor: '#A58BFF' }]}
          color="#0D0B14"
