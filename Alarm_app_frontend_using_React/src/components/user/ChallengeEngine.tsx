...
export const ChallengeEngineView: React.FC = () => {
  ...
  return (
    <div className="w-full bg-[#0B0E1B] text-slate-100 rounded-3xl p-3.5 sm:p-8 shadow-2xl space-y-6 sm:space-y-8 border border-slate-800/80 min-h-[580px] relative overflow-hidden font-sans">
      ...
      <div className="relative w-full max-w-4xl mx-auto my-4 flex flex-col items-center justify-center">
        
        {/* Center Glowing Node */}
        <div className="my-4 z-20 flex items-center justify-center">
          <div className="w-28 h-28 sm:w-40 sm:h-40 rounded-full bg-gradient-to-tr from-amber-500 via-amber-400 to-amber-300 shadow-[0_0_50px_rgba(245,158,11,0.4)] flex flex-col items-center justify-center p-3 text-center border-4 border-amber-200/50 animate-pulse">
            <span className="text-[10px] sm:text-xs font-black text-slate-950 uppercase tracking-wider leading-tight">
              COGNITIVE<br />CHALLENGE<br />ENGINE
            </span>
          </div>
        </div>

        {/* Node Buttons Grid Layout for mobile and desktop */}
        <div className="w-full relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs font-mono">
          {NODES.map((node) => {
            const isSelected = selectedNode?.id === node.id;
            return (
              <button
                key={node.id}
                onClick={() => generatePuzzle(node)}
                className={`p-3 rounded-2xl border transition-all text-left bg-[#12172D]/90 backdrop-blur-md hover:border-amber-400 active:scale-95 flex flex-col justify-between ${
                  isSelected ? 'border-amber-400 ring-2 ring-amber-400/30 bg-[#1e233d]' : 'border-slate-800 text-slate-200 hover:bg-[#1a203a]'
                }`}
              >
                <div className="font-bold text-white text-xs sm:text-sm leading-snug break-words">
                  {node.label}
                </div>
                <div className="text-[10px] text-amber-400 mt-2 flex items-center justify-between">
                  <span>{node.category}</span>
                  <span className="text-[9px] font-mono bg-amber-400/10 px-1.5 py-0.5 rounded text-amber-300">Start ►</span>
                </div>
              </button>
            );
          })}
