...
async def validate_verification(
    ...
    if request.is_correct:
        ...
        if new_count >= ver.get("required_correct_answers", 1):
            ...
            # TRIGGER MILESTONE 3 INTELLIGENCE ENGINE
            from app.services.intelligence_service import IntelligenceService
            import asyncio
            asyncio.create_task(IntelligenceService.process_post_alarm_analytics(db, str(current_user.id), request.verification_id))
    else:
        # The user got it wrong.
        # Do nothing to the correct answers count (no longer resetting to 0 per new requirements).
        pass
